...
export const updateUser = (id: number, data: Partial<User>): Promise<User> => api.patch(`/users/update/${id}`).then(res => res.data);
export const deleteUser = (id: number): Promise<void> => api.delete(`/users/softDelete/${id}`).then(res => res.data);