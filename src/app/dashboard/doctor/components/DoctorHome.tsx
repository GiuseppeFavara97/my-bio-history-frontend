...
import { Allergy, MedicalRecord, Patient, Vaccine } from "@/Types/Types";
import { getPatients } from "@/lib/api/patient";

import { getMedicalRecord } from "@/lib/api/medicalRecord";
...
export default function DoctorHome() {
  ...
  useEffect(() => {
    ...
    async function fetchMedicalRecord(id: number) {
      ...
      const data: MedicalRecord = await getMedicalRecord(id);
  ...
  console.log("medicalRecord", medicalRecord, "di paziente", selectedPatient?.id);
  return (
    <main className="flex h-full w-full flex-col overflow-hidden">
      <header className="h-15 flex w-full items-center justify-between bg-blue-800 text-white">
        <p className="mx-10 text-3xl font-medium">Bentornato Dottor White</p>
      ...
      <div id="right" className="flex w-full flex-1 ">
        <aside className="flex h-full w-3/12 items-center justify-center ">
          <div className="h-11/12 flex w-10/12 flex-col rounded-2xl bg-gray-400/30">
            <div id="filtersection" className="flex w-full items-center gap-2 p-2">
              ...
              />
              <SlidersHorizontal className="shrink-0 cursor-pointer" />
            </div>
        ...
        <div className="flex flex-1 flex-col outline " id="rightpart">
          <div className="m-10 flex w-10/12 flex-col self-center rounded-2xl p-3 outline-2 outline-black">
          ...
          <div className=" m-8 flex flex-1 ">
            <div
              className="flex-3
                            mx-10 flex flex-col rounded-2xl bg-gray-400/20 p-2 outline"
            >
            ...
            <div className="flex-1 rounded-2xl bg-gray-400/20 p-2 outline">
